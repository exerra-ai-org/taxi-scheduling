-- 000_baseline.sql
--
-- Full base schema. Hand-derived from drizzle/0000_hard_eternals.sql with
-- two changes so it is safe to run against an EXISTING DB (no-op) and a
-- FRESH DB (full create) alike:
--
--   • CREATE TYPE wrapped in DO $$ ... WHEN duplicate_object THEN null
--     (CREATE TYPE has no IF NOT EXISTS in older pg versions).
--   • CREATE TABLE / INDEX already use IF NOT EXISTS in the Drizzle
--     output; ALTER TABLE ADD CONSTRAINT already uses DO blocks with
--     duplicate_object guards — left as-is.
--
-- DO NOT wrap the whole file in a transaction: CREATE TYPE outside DO
-- blocks misbehaves inside transactions in some pg versions; each
-- statement is independently idempotent.

-- ── enums ──────────────────────────────────────────────────────────────
DO $$ BEGIN
 CREATE TYPE "public"."booking_status" AS ENUM('scheduled', 'assigned', 'en_route', 'arrived', 'in_progress', 'completed', 'cancelled');
...
DO $$ BEGIN
 CREATE TYPE "public"."driver_assignment_role" AS ENUM('primary', 'backup');
EXCEPTION WHEN duplicate_object THEN null; END $$;

DO $$ BEGIN
 CREATE TYPE "public"."payment_intent_type" AS ENUM('payment_intent', 'setup_intent');
EXCEPTION WHEN duplicate_object THEN null; END $$;

DO $$ BEGIN
 CREATE TYPE "public"."payment_status" AS ENUM('unpaid', 'pending', 'requires_action', 'authorized', 'captured', 'partially_refunded', 'refunded', 'failed', 'disputed', 'uncollectible');
EXCEPTION WHEN duplicate_object THEN null; END $$;

DO $$ BEGIN
 CREATE TYPE "public"."refund_reason" AS ENUM('requested_by_customer', 'duplicate', 'fraudulent', 'service_failure', 'route_change', 'cancellation_full', 'cancellation_partial', 'other');
EXCEPTION WHEN duplicate_object THEN null; END $$;
...
DO $$ BEGIN
 CREATE TYPE "public"."vehicle_class" AS ENUM('regular', 'comfort', 'max');
EXCEPTION WHEN duplicate_object THEN null; END $$;
...
-- ── foreign keys (DO blocks tolerate "already exists") ─────────────────
DO $$ BEGIN
...
CREATE INDEX IF NOT EXISTS "idx_bookings_scheduled_at" ON "bookings" USING btree ("scheduled_at");
CREATE INDEX IF NOT EXISTS "idx_bookings_status_sched" ON "bookings" USING btree ("status","scheduled_at");
CREATE INDEX IF NOT EXISTS "idx_bookings_payment_status" ON "bookings" USING btree ("payment_status");
CREATE INDEX IF NOT EXISTS "idx_bookings_pay_status_sched" ON "bookings" USING btree ("payment_status","scheduled_at");
CREATE INDEX IF NOT EXISTS "idx_disputes_booking" ON "disputes" USING btree ("booking_id");
CREATE INDEX IF NOT EXISTS "idx_disputes_status" ON "disputes" USING btree ("status");
CREATE INDEX IF NOT EXISTS "idx_da_booking" ON "driver_assignments" USING btree ("booking_id");
CREATE INDEX IF NOT EXISTS "idx_da_driver_active" ON "driver_assignments" USING btree ("driver_id","is_active");
CREATE INDEX IF NOT EXISTS "idx_da_booking_active" ON "driver_assignments" USING btree ("booking_id","is_active");
...
CREATE INDEX IF NOT EXISTS "idx_hb_last" ON "driver_heartbeats" USING btree ("last_heartbeat_at");
...
CREATE INDEX IF NOT EXISTS "idx_incidents_booking_id" ON "incidents" USING btree ("booking_id");
CREATE INDEX IF NOT EXISTS "idx_notif_subs_user_id" ON "notification_subscriptions" USING btree ("user_id");
CREATE INDEX IF NOT EXISTS "idx_payments_booking" ON "payments" USING btree ("booking_id");
CREATE INDEX IF NOT EXISTS "idx_payments_customer" ON "payments" USING btree ("customer_id");
CREATE INDEX IF NOT EXISTS "idx_payments_status" ON "payments" USING btree ("status");
CREATE INDEX IF NOT EXISTS "idx_payments_charge" ON "payments" USING btree ("stripe_charge_id");
CREATE INDEX IF NOT EXISTS "idx_refunds_booking" ON "refunds" USING btree ("booking_id");
...
CREATE INDEX IF NOT EXISTS "idx_reviews_driver_id" ON "reviews" USING btree ("driver_id");
CREATE INDEX IF NOT EXISTS "idx_users_role" ON "users" USING btree ("role");
CREATE INDEX IF NOT EXISTS "idx_users_magic_link_token" ON "users" USING btree ("magic_link_token");
CREATE INDEX IF NOT EXISTS "idx_users_reset_password_token" ON "users" USING btree ("reset_password_token");
CREATE INDEX IF NOT EXISTS "idx_users_invitation_token" ON "users" USING btree ("invitation_token");
CREATE INDEX IF NOT EXISTS "idx_webhook_events_type" ON "webhook_events" USING btree ("type");
